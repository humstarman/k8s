...
if [ "$JVM_VERSION" \< "1.8" ] ; then
    echo "Cassandra 3.0 and later require Java 8u40 or later in 1."
    exit 1;
fi

if [ "$JVM_VERSION" \< "1.8" ] && [ "$JVM_PATCH_VERSION" -lt 131 ] ; then
    echo "Cassandra 3.0 and later require Java 8u40 or later in 2."
    exit 1;
fi
