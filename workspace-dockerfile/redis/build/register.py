...
def parse_opts(parser):
    ...
    parser.add_option("-p","--port",action="store",type="string",dest="port",default="6379",help="Todo")
...
def put_v_with_ttl(k,v,t=options.ttl):
    logger.info("key: %s" % k)
    try:
        client = get_etcd_client()
        client.write("%s" % k, v, ttl=t) 
    except: pass
    sleep(options.slot)
    ret = client.read("%s" % k).value
    logger.debug("ret: %s" % ret)
...
def main():
    ...
    put_v_with_ttl('/register/%s' % options.ip,options.port)
    try:
