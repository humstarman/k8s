...
def parse_opts(parser):
    ...
    parser.add_option("--app",action="store",type="str",dest="APP",default="zookeeper",help="todo")
...
def main():
    ...
    ip_id_pairs_not_sorted = [(ip_id.strip().split(':')[0],int(ip_id.strip().split(':')[1])) for ip_id in ip_id_lst]
    logger.debug("IP - ID pairs not sorted:")
    logger.debug(ip_id_pairs_not_sorted)
    ip_id_pairs = sorted(ip_id_pairs_not_sorted,key = lambda x: x[-1]) 
    logger.debug("IP - ID pairs sorted:")
    logger.debug(ip_id_pairs)
    
    with open(options.HOSTS,"a+") as f1, open (options.ZOOCFG,"a+") as f2:
        for (_ip,_id) in ip_id_pairs:
            string1 = "%s\tzoo%d\n" % (_ip,_id)
            f1.write(string1)
            if _id == options.ID:
                string2 = "server.%d=0.0.0.0:2888:3888\n" % _id 
            else:
                string2 = "server.%d=zoo%d:2888:3888\n" % (_id,_id) 
            f2.write(string2)
